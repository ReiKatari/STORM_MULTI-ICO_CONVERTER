...
def create_test_ico_ascending():
    source_img = "stormmultiicoconverter.jpg"
    out_path = "test_ascending.ico"
    ...
    try:
        with Image.open(source_img) as img:
            ...
            # Save
            resized_images[0].save(out_path, format='ICO', append_images=resized_images[1:])
            print(f"Created {out_path}")
            
    except Exception as e:
        print(f"Creation failed: {e}")

def verify_ico_ascending():
    out_path = "test_ascending.ico"
    ...
    try:
        with Image.open(out_path) as img:
            ...
            # Check if 16x16 is first (Ascending order)
            if img.size == (16, 16):
                 print("SUCCESS: 16x16 is first (Ascending).")
            else:
...
if __name__ == "__main__":
    create_test_ico_ascending()
    verify_ico_ascending()
